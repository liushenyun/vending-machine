...
Component({
  ...
  methods: {
    ...
    _yearChange: function (newYear, oldYear) {
      console.log('newYearnewYear', newYear)
    ...
    _showLunar: function (newConfig, oldConfig) {
      console.log('newConfig, oldConfig', newConfig, oldConfig)
      this.setData({
        lunar: !!newConfig
      });
    },
    ...
    _setCalendarData: function (year, month) {
      ...
      console.log('days[j]', days)

      /**
       * 设置农历
       */
      // let showAfter = false;
      // let cutDate = '2018-08-15';
      for (let i = 0; i < days.length; i++) {
        const item = days[i];
        const solarDay = item.day;
        const solarMonth = item.month;
        const solarYear = year;
        const curDate = solarYear + '-' + solarMonth + '-' + solarDay;
        // console.log('solarYear, solarMonth, solarDay', solarYear, solarMonth, solarDay)
        if (solarDay > 0) {
          ...
          // 添加今天标志
          if (new Date(curDate).getTime() == new Date(this._getTodayDateTime()).getTime()) {
            days[i].isToday = true;
          }
          // if (!showAfter && (new Date(cutDate).getTime() < new Date(curDate).getTime())) {
          //   days[i].day = -1;
          //   days[i].background = 'transparent';
          // }
        }
      }
